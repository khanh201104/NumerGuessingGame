...
public class MainGUI {
    ...
    // BIẾN MỚI: Cờ theo dõi trạng thái Developer Mode
    ...
    private static void createAndShowGUI() {
        ...
        btnPlay.addActionListener(e -> {
            ...
            isDevMode = false; // Reset cờ Dev Mode khi chơi ván mới
            startNewLevel();
    ...
    private static void startNewLevel() {
        ...
        if (infoLabel != null) {
            // TÍCH HỢP DEV MODE: Tự động hiển thị đáp án nếu cờ DevMode đang bật
            String baseText = "Khoảng đoán: [ " + currentMin + "  -  " + currentMax + " ]";
            if (isDevMode) {
                infoLabel.setText(baseText + " | ĐÁP ÁN: " + secretNumber);
            } else {
    ...
    private static JPanel createGamePanel(JFrame parentFrame) {
        ...
        btnShop.addActionListener(e -> {
            ...
            if (choice == 0) {
                ...
                if (gold < 30) { CustomDialogs.showMessage(parentFrame, "LỖI", "Bạn không đủ Vàng!", CustomDialogs.TYPE_ERROR); }
                    ...
                    // Cập nhật lại UI Cắt khoảng đoán có tính đến DevMode
                    String baseText = "Khoảng đoán: [ " + currentMin + "  -  " + currentMax + " ]";
                    if (isDevMode) {
                        infoLabel.setText(baseText + " | ĐÁP ÁN: " + secretNumber);
                    } else {
        ...
        // =====================================================================
        // SỰ KIỆN NÚT ĐOÁN SỐ (Bao gồm các mã Cheat Code ẩn)
        // =====================================================================
        btnGuess.addActionListener(e -> {
            SoundManager.playSound("click.wav");
            String input = inputField.getText().trim();

            // 1. CHEAT CODE: Bật Developer Mode (Hiện đáp án)
            if (input.equalsIgnoreCase("devmode")) {
                isDevMode = true;
                infoLabel.setText("Khoảng đoán: [ " + currentMin + "  -  " + currentMax + " ] | ĐÁP ÁN: " + secretNumber);
                inputField.setText("");
                return;
            }

            // 2. CHEAT CODE: Tắt Developer Mode (Ẩn đáp án)
            if (input.equalsIgnoreCase("undev")) {
                isDevMode = false;
            ...
            // 3. CHEAT CODE: Qua màn nhanh (Tới màn 30 thì tự động thắng)
            ...
            // 4. Các phím tắt và kiểm tra rỗng
